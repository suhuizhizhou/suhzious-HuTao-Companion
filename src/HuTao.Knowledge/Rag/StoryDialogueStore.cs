...
public sealed class StoryDialogueLine
{
    ...
    [JsonIgnore] public string EvidenceId => EvidenceKind switch
    {
        ...
        // 角色档案的 id 前缀**从档案文件名派生**（character/hutao.json → hutao），
        // 而不是写死 "hutao"。对胡桃结果完全一致（character:hutao:…），
        // 换角色时自然变成 character:lacrimosa:…，不需要改代码。
        // ⚠️ 这个 id 是**评测数据集的契约**（benchmark 的金标直接引用它），所以派生化必须保持原值。
        "character_story" => $"character:{CharacterStoryStem}:{TextHash}:{Sequence}",
        _ => $"textmap:{ChapterId}:{SubquestIndex}:{LineId}:{Variant}",
    };

    /// <summary>角色档案名的词干，用作证据 id 前缀。</summary>
    [JsonIgnore] public string CharacterStoryStem
    {
        get
        {
            var name = Path.GetFileNameWithoutExtension(SourceFile ?? "");
            return string.IsNullOrWhiteSpace(name) ? "story" : name;
        }
    }
