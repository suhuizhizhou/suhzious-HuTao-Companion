...
namespace HuTao.Dialogue.Storage;

/// <summary>用户明确请角色记住的一条重要事项。</summary>
...
/// <summary>
/// 重要事项彩蛋：只接受用户显式的“记住/提醒我”指令，保存于本地 JSON，
/// 并按截止时间与冷却间隔向 Agent 提供少量候选提醒。
///
/// **唤醒词（角色名/自称）由构造参数传入**，不再写死「胡桃」——
/// 否则换个角色，「安魂曲，记住…」这种说法就完全认不出来。
/// </summary>
public sealed class ImportantMemoryStore
{
    private readonly Regex _rememberPattern;
    private readonly Regex _forgetPattern;

    /// <param name="wakeWords">
    /// 允许出现在指令前的称呼（角色名 + 自称，如 胡桃 / 本堂主 / 桃桃）。
    /// 为空时不接受任何前缀，只认裸指令。
    /// </param>
    public ImportantMemoryStore(string path, IEnumerable<string>? wakeWords = null)
    {
        _path = Path.GetFullPath(path);
        var prefix = BuildWakeWordPrefix(wakeWords);
        _rememberPattern = new Regex(
            $@"^\s*{prefix}(?:请)?(?:帮我)?(?:记住|记一下|记得|提醒我)\s*[：:]?\s*(?<content>.+?)\s*$",
            RegexOptions.Compiled | RegexOptions.CultureInvariant);
        _forgetPattern = new Regex(
            $@"^\s*{prefix}(?:请)?(?:帮我)?(?:忘掉|忘记|删掉|删除)\s*(?:关于)?\s*(?<content>.+?)\s*$",
            RegexOptions.Compiled | RegexOptions.CultureInvariant);
        _memories = Load();
    }

    /// <summary>把唤醒词拼成可选的 `(?:词1|词2)[，,、 ]*` 前缀；没有唤醒词时返回空串。</summary>
    private static string BuildWakeWordPrefix(IEnumerable<string>? wakeWords)
    {
        var words = (wakeWords ?? [])
    ...
    public string PreparePrompt(string? userInput, bool isProactive, DateTimeOffset now)
    {
        lock (_gate)
        {
            ...
            {
                var forget = _forgetPattern.Match(userInput);
                if (forget.Success)
                    return Forget(forget.Groups["content"].Value.Trim());

                var remember = _rememberPattern.Match(userInput);
                if (remember.Success)
