...
public sealed class LocalDiagnosticLog
{
    ...
    public void Write(string stage, Exception error)
    {
        try
        {
            ...
            // 子进程失败走 IProcessFailure 契约，而不是 `is TtsProcessException`——
            // 后者会让 Foundation 层反向依赖 Voice 层，拆程序集时直接成环。见 IProcessFailure 的说明。
            var failure = error as IProcessFailure;
            var row = JsonSerializer.Serialize(new
            {
                ...
                stderr = failure is null ? null : Redact(failure.Diagnostic)
            });
            Append(row);
        }
        catch
