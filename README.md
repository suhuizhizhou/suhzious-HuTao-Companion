...
默认进入胡桃；点击右上角角色按钮会展开**胡桃、芙宁娜、可莉、安魂曲**的选择列表，人设、声线、主题和聊天记录随角色切换。输入框打字回车即可对话；缺少音频时自动使用文字模式。

最后一位来自《异环》（Neverness To Everness），人设包、剧情语料与原声全部由 `tools/nte` 从本机游戏资源提取，与桌宠既有规范完全一致——**跨作品只是多一条配置，检索与语音链路一条都没改**。安魂曲 450 条原声（363 MB / 72 分钟），能用真原声说话；她的语音在游戏的**语言分包**（`TagPatchPaks`）里，工具需要多挂一个容器目录，见 [`tools/nte/README.md`](tools/nte/README.md) §2.5（`tools/` 不入库）。
