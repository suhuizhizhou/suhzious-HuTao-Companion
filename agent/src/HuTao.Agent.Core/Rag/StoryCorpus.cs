...
internal sealed class StoryCorpus
{
    ...
    public static StoryCorpus Load(string dialogueRoot)
    {
        ...
        var memory = Path.Combine(Path.GetDirectoryName(dialogueRoot)!, "character", "hutao.json");
        if (File.Exists(memory)) Read(memory, json =>
        {
            using var doc = JsonDocument.Parse(json);
            foreach (var story in doc.RootElement.GetProperty("stories").EnumerateArray())
            {
                ...
                foreach (var paragraph in story.GetProperty("paragraphs").EnumerateArray())
                {
                    ...
                    lines.Add(new StoryDialogueLine
                    {
                        ...
                        SourceFile = "character/hutao.json",
                        Speaker = "角色资料叙述"
                    });
                }
            }
